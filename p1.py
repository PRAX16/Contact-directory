...
def save():
    ...
    data_dict = {'name':name,'phone':phone, 'address':address,'dob':dob,'Gmail':gmail,'Insta':insta,'Facebook':facebook, 'snapchat':snapchat}
...
def create_window1():
    global sc_tb,temp2, select, name_tb, phone_number_tb, address_tb, spinbox_date, spinbox_month, spinbox_year, temp, spinbox_cl, e_tb, ig_tb, fb_tb
    create = Toplevel()
    create.title('Create new contact')

    create.geometry("650x600")
    create['bg'] = "grey"
    ...
    image5 = Image.open("C:\\Users\\Admin\\Downloads\\snap.jpg")
    ...
    label5 = Label(frame2, image=img5)
    label5.image = img5
    ...
    sc_tb['width'] = 23
    sc_tb['fg'] = "white"
    sc_tb.configure(bg='#E6D530',font=('Calibri (Body)',12,'bold'),bd=5,relief=GROOVE,justify='center')
    sc_tb.grid(row=4,column=1,pady=2,padx=20)
...
def opensnapcode():
    global fb_tb, dict_list, sc_tb
    file_data = open("contact_list.txt", "r")
    lines = file_data.readlines()
    for i in range(0,len(dict_list)):
        scc = dict_list[i].get('snapchat')

    snapp = ("https://story.snapchat.com/@"+scc+"/")
    webbrowser.open(snapp)
...
def view():
    ...
    #Snap button image
    image15 = Image.open("C:\\Users\\Admin\\Downloads\\snap.jpg")
 
    #Resize the image
    resize_image15 = image15.resize((70,70))
 
    img15 = ImageTk.PhotoImage(resize_image15)
    
    label115 = Label(frame2_4, image=img15)
    label115.image = img15
    label115.grid(row=5,column=4)
    ...
    #SNAPCHAT BUTTON
    button_snap = Button(frame2_4, command = opensnapcode)
    button_snap.grid(row=5, column = 4)      
